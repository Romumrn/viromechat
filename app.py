...
# Local dev secrets (ALBERT_API_KEY, PASSWORD_ENABLED, ACCESS_CODE) — see
# .env.app.example. On Streamlit Community Cloud, st.secrets is used instead
# and always takes priority (see _get_secret below).
...
def _get_bool_secret(key: str, default: bool = False) -> bool:
    """Same as _get_secret, but normalized to bool — st.secrets returns a
    real bool for TOML `true`/`false`, while a .env file only has strings."""
    val = st.secrets.get(key)
    if val is None:
...
def check_password():
    """Simple password gate using st.secrets / .env.app."""
    ...
    if st.button("Login"):
        if pwd == _get_secret("ACCESS_CODE"):
...
def main():
    ...
    with st.sidebar:
        st.title("Viromech@t 🦠")
        st.caption("""
A chatbot to explore viral metagenomic data from the Virome@tlas project.
